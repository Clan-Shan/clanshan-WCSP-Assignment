...
WegVect = pow2([log2(mod_size)-1:-1:0]); % Bit to decimal 用

trial_loop = [2000 2000 2000 2000 2000 2000 5000 5000 10000 20000 40000];

% ====== Monte Carlo process ===============
for snr = 1:length(SNR)
        Trial = trial_loop(snr); % 不同 SNR 對應不同模擬次數（越高 SNR 模擬次數越多）
    for t = 1:Trial
        ...
        data = WegVect * raw_bits; % bit to decimal (0~3)
        inputSymbols = qammod(data, mod_size); % QAM 調變

        % -------- transmit data symbol------------------
        tx_MQAM = 1/sqrt(2)*inputSymbols.'; % 標準化能量
